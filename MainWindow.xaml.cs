...
namespace PolarisBiosEditor
{
    public partial class MainWindow : Window
    {
        public MainWindow()
        {
            ...
            /*
            MessageBox.Show(
                "Modifying your BIOS is dangerous and could cause irreversible damage to your GPU.\n" +
                "Using a modified BIOS may void your warranty.\n" +
                "The author will not be held accountable for your actions.",
                "DISCLAIMER", MessageBoxButton.OK, MessageBoxImage.Warning
            );
            */
        }
        ...
        private void OpenFileDialog_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                ...
                using (BinaryReader br = new BinaryReader(fileStream))
                {
                    ...
                    {
                        msgSuported = MessageBox.Show("Unsupported DeviceID 0x" + deviceID + " - Continue?",
                    ...
                    if (msgSuported == MessageBoxResult.Yes)
                    {
                        ...
                        {
                            atom_mclk_entries[i] = fromBytes<ATOM_MCLK_ENTRY>(buffer.Skip(atom_mclk_table_offset +
                                                                                          get_size<ATOM_MCLK_TABLE>() +
                                                                                          get_size<ATOM_MCLK_ENTRY>() * i).ToArray());
                        }
                        ...
                        {
                            atom_sclk_entries[i] = fromBytes<ATOM_SCLK_ENTRY>(buffer.Skip(atom_sclk_table_offset +
                                                                                          get_size<ATOM_SCLK_TABLE>() +
                                                                                          get_size<ATOM_SCLK_ENTRY>() * i).ToArray());
                        }

                        atom_vddc_table_offset = atom_data_table.PowerPlayInfo + atom_powerplay_table.usVddgfxLookupTableOffset;
                        ...
                        {
                            atom_vddc_entries[i] = fromBytes<ATOM_VOLTAGE_ENTRY>(buffer.Skip(atom_vddc_table_offset +
                                                                                             get_size<ATOM_VOLTAGE_TABLE>() +
                                                                                             get_size<ATOM_VOLTAGE_ENTRY>() * i).ToArray());
                        }

                        atom_vram_info_offset = atom_data_table.VRAM_Info;
                        byte[] atom_vram_info_bytes = buffer.Skip(atom_vram_info_offset).ToArray();
                        atom_vram_info = fromBytes<ATOM_VRAM_INFO>(atom_vram_info_bytes);
                        atom_vram_entries = new ATOM_VRAM_ENTRY[atom_vram_info.ucNumOfVRAMModule];
                        var atom_vram_entry_offset = atom_vram_info_offset + get_size<ATOM_VRAM_INFO>();
                        ...
                        byte[] needle = { 0x40, 0x9C, 0x00 };
                        atom_vram_timings_offset = find_bytes(atom_vram_info_bytes, needle);
                        if (atom_vram_timings_offset == -1)
                        {
                            MessageBox.Show("Failed to find 400MHz strap");
                        }
                        else
                        {
                            atom_vram_timings_offset += atom_vram_info_offset;
                            atom_vram_timing_entries = new ATOM_VRAM_TIMING_ENTRY[16];
                            for (var i = 0; i < 32; i++)
                            {
                                atom_vram_timing_entries[i] = fromBytes<ATOM_VRAM_TIMING_ENTRY>(buffer.Skip(atom_vram_timings_offset +
                                                                                                            get_size<ATOM_VRAM_TIMING_ENTRY>() * i).ToArray());

                                // atom_vram_timing_entries have an undetermined length
                                // attempt to determine the last entry in the array
                                if (atom_vram_timing_entries[i].ulClkRange.All(b => b == 0x0))
                                {
                        ...
                        init_table_powerplay();

                        init_table_powertune();
                        ...
                        for (var i = 0; i < atom_mclk_table.ucNumEntries; i++)
                        {
                            tableMEMORY.Items.Add(new
                            {
                                ...
                                MV = atom_mclk_entries[i].usVddci
                            });
                        }
            ...
            if (SaveFileDialog.ShowDialog() == true)
            {
                ...
                using (BinaryWriter bw = new BinaryWriter(fs))
                {
                    ...
                    for (var i = 0; i < tableMEMORY.Items.Count; i++)
                    {
                        ...
                        atom_mclk_entries[i].usVddci = (UInt16)mv;
                    }
                    ...
                    for (var i = 0; i < tableVRAM_TIMING.Items.Count; i++)
                    {
                        ...
                        var index = (FindByName("INDEX", container) as TextBlock).Text;
                        ...
                        atom_vram_timing_entries[i].ulClkRange = BitConverter.GetBytes(mhz).Take(3).ToArray();
                        atom_vram_timing_entries[i].ucIndex = Convert.ToByte(index);
                    ...
                    for (var i = 0; i < atom_mclk_table.ucNumEntries; i++)
                    {
                        setBytesAtPosition(buffer,
                                           atom_mclk_table_offset + get_size<ATOM_MCLK_TABLE>() + get_size<ATOM_MCLK_ENTRY>() * i,
                                           getBytes(atom_mclk_entries[i]));
                    }

                    for (var i = 0; i < atom_sclk_table.ucNumEntries; i++)
                    {
                        setBytesAtPosition(buffer,
                                           atom_sclk_table_offset + get_size<ATOM_SCLK_TABLE>() + get_size<ATOM_SCLK_ENTRY>() * i,
                                           getBytes(atom_sclk_entries[i]));
                    }

                    for (var i = 0; i < atom_vddc_table.ucNumEntries; i++)
                    {
                        setBytesAtPosition(buffer,
                                           atom_vddc_table_offset + get_size<ATOM_VOLTAGE_TABLE>() + get_size<ATOM_VOLTAGE_ENTRY>() * i,
                                           getBytes(atom_vddc_entries[i]));
                    }

                    for (var i = 0; i < atom_vram_timing_entries.Length; i++)
                    {
                        setBytesAtPosition(buffer,
                                           atom_vram_timings_offset + get_size<ATOM_VRAM_TIMING_ENTRY>() * i,
                                           getBytes(atom_vram_timing_entries[i]));
                    }
        ...
        private void listVRAM_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (listVRAM.SelectedIndex >= 0 && listVRAM.SelectedIndex < listVRAM.Items.Count)
            {
                ...
                var vram = atom_vram_entries[atom_vram_index];
                
                tableVRAM.Items.Add(new
                {
                    NAME = "VendorID",
                    VALUE = "0x" + vram.ucMemoryVenderID.ToString("X")
                });
                tableVRAM.Items.Add(new
                {
                    NAME = "Size (MB)",
                    VALUE = vram.usMemorySize
                });
                tableVRAM.Items.Add(new
                {
                    NAME = "Density",
                    VALUE = "0x" + vram.ucDensity.ToString("X")
                });
                tableVRAM.Items.Add(new
                {
                    NAME = "Type",
         ...
         */
        public int find_bytes(byte[] haystack, byte[] needle)
        {
        ...
        public void setBytesAtPosition(byte[] dest, int offset, byte[] src)
        {
        ...
        private void init_table_powerplay()
        {
            //tablePOWERPLAY.Items.Clear();
        ...
        private void init_table_powertune()
        {
            //tablePOWERTUNE.Items.Clear();
        ...
        int atom_vram_timings_offset;   // offset to start of timings
        ...
        unsafe struct ATOM_POWERTUNE_TABLE
        {
            ...
            public UInt16 usReserve1;
            public UInt16 usReserve2;
        };
